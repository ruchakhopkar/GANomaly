...
def getInputData():
    ...
    edw_query = f"""select 
                    ...
                    wafercode in ('6P', '2W', 'I1') and (lp_slope_target<1)
                """
    reli_df = None
    nrm_ifc = DAO_Interface(cfg.orc_username,cfg.orc_passwd,cfg.orc_hostname,cfg.orc_sid,cfg.orc_port,cfg.orc_encoding)
    reli_df = nrm_ifc.read(edw_query)
...
def keepLatestData(reli_df):
    ...
    edw_query = f"""select 
                    ...
                    wafercode in ('6P', '2W', 'I1')
                """
    my_df = None
    nrm_ifc = DAO_Interface(cfg.orc_username,cfg.orc_passwd,cfg.orc_hostname,cfg.orc_sid,cfg.orc_port,cfg.orc_encoding)
    my_df = nrm_ifc.read(edw_query)
    hds = set(reli_df['HD_NUM'].to_list()).difference(set(my_df['HD_NUM'].to_list()))
    reli_df = reli_df[reli_df['HD_NUM'].isin(hds)]
    ...
    print(len(dates))
    return images, wafercode, wfr_hamr_design, slider_lot_num, wafer_substrate_lot_id, dates, hd_num, reli_df

def storeData(images, hd_num, reli_df):
        #remove existing test files and output directory
        try:
            os.system('rm -rf /home/ruchak/ganomaly-master/data/cdsem/test/1.abnormal')
        except:
            pass
        try:
            os.system('rm -rf /home/ruchak/ganomaly-master/output/ganomaly/cdsem/test/images')
        ...
        '''if not(os.path.exists(os.path.join('/home/ruchak/ganomaly-master/data/cdsem/test', '/output_0.23'))):
            os.mkdir('/home/ruchak/ganomaly-master/data/cdsem/test' + '/outputs_0.23')
        if not(os.path.exists(os.path.join('/home/ruchak/ganomaly-master/data/cdsem/test', '/outputs_0.42'))):
            os.mkdir('/home/ruchak/ganomaly-master/data/cdsem/test' + '/outputs_0.42')'''
...
def storeResults(images, wafercode, wfr_hamr_design, slider_lot_num, wafer_substrate_lot_id, dates):
    ...
    df = pd.DataFrame()
    x = sorted(os.listdir('/home/ruchak/ganomaly-master/data/cdsem/test/1.abnormal/'))
    df['Filename'] = x
    df['PREDICTIONS'] = an
    ...
    #df.to_csv('/extrastg/mdfs14/hamr-analysis/image_repo/slider_CDSEM_anomaly/marlin results/results.csv', index = False)
    
    definitely_anomalous = df.iloc[np.where(df['DEFINITELY_ANOMALOUS']==1)[0], :]['HD_NUM']
    
    '''for file in definitely_anomalous:
        shutil.copy('/extrastg/mdfs14/hamr-analysis/image_repo/slider_CDSEM_anomaly/marlin results/1.abnormal/' +file + '.png',
                    '/extrastg/mdfs14/hamr-analysis/image_repo/slider_CDSEM_anomaly/marlin results/outputs_0.42/'+file + '.png')
    
    hard_thresholding = df.iloc[np.where(df['HARD_THRESHOLDING_RESULTS']==1)[0], :]['HD_NUM']
    
    for file in hard_thresholding:
        shutil.copy('/extrastg/mdfs14/hamr-analysis/image_repo/slider_CDSEM_anomaly/marlin results/1.abnormal/' +file+'.png',
                    '/extrastg/mdfs14/hamr-analysis/image_repo/slider_CDSEM_anomaly/marlin results/outputs_0.23/'+file + '.png')'''
    return df
def job():

    os.environ['MKL_THREADING_LAYER'] = 'GNU'
    reli_df = getInputData()
    images, wafercode, wfr_hamr_design, slider_lot_num, wafer_substrate_lot_id, dates, hd_num, reli_df = keepLatestData(reli_df)
    

    if len(reli_df)>0:
        
        #storeData(images, hd_num, reli_df)
        #get predictions
        ...
        store_df = storeResults(images, wafercode, wfr_hamr_design, slider_lot_num, wafer_substrate_lot_id, dates)
