    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            if res[self.opt.metric] > best_auc:
                ...
                np.save(self.opt.outf + '/'+self.opt.name + '/test/gt_scores.npy', gt)
                np.save(self.opt.outf + '/'+self.opt.name + '/test/an_scores.npy', an)
            else:
                aucs += 1
            #if aucs>6:
            #    break
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                path = self.opt.outf + "/{}/{}/train/weights/netG.pth".format(self.name.lower(), self.opt.dataset)
                print(path)
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                if self.opt.save_test_images:
                    ...
                    #print(self.opt.datasets_names['test'][i])
                    vutils.save_image(fake, dst+'/'+self.opt.datasets_names['test'][i][0].split('/')[-1], normalize=True)
            ...
            print(auc)
            #np.save('test_gt.npy', self.gt_labels.cpu().numpy())
            #np.save('test_an_scores.npy', self.an_scores.cpu().numpy())
            if self.opt.display_id > 0 and self.opt.phase == 'test':
                counter_ratio = float(epoch_iter) / len(self.dataloader['test'].dataset)
                self.visualizer.plot_performance(self.epoch, counter_ratio, performance)
            return performance, self.gt_labels.cpu().numpy(), self.an_scores.cpu().numpy()
